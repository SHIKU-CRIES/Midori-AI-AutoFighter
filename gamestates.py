        ...
        while running:
            ...
            if len(foelist) > 0:
                ...
                for i, person in enumerate(foelist):
                    ...
                    if bleed_mod > 1.5:
                        person.RushStat = 0
                        person.take_damage(bleed_mod, bleed_mod * 1000)
                        person.gain_damage_over_time(damageovertimetype("Bleed", person.MHP * (0.001 * bleed_mod), max(10, round(1.5 ** bleed_mod)), Generic, person.PlayerName, round(1.2 * bleed_mod)), 0.2 * bleed_mod)
            ...
            if len(playerlist) > 0:
                for i, person in enumerate(playerlist):
                    ...
                    if bleed_mod > 1.5:
                        person.RushStat = 0
                        person.take_damage(bleed_mod, bleed_mod * 1000)
                        person.gain_damage_over_time(damageovertimetype("Bleed", person.MHP * (0.001 * bleed_mod), max(10, round(1.5 ** bleed_mod)), Generic, person.PlayerName, round(1.2 * bleed_mod)), 0.25 * bleed_mod)
