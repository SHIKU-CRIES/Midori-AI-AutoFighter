    ...
    async def apply(self, party) -> None:  # type: ignore[override]
        ...
        def _on_battle_end():
            ...
            if battle_deaths == 0:
                ...
                BUS.emit("card_effect", self.id, None, "guardian_mitigation_pending", 1, {
                    "mitigation_bonus": 1,
            ...
            if target in party.members:
                ...
                if mitigation_bonus_pending:
                    ...
                    for member in party.members:
                        # Apply +1 small mitigation (theoretical implementation)
                        import logging
                        log = logging.getLogger(__name__)
                        log.debug("Guardian Shard mitigation bonus: +1 mitigation to %s", member.id)
                        BUS.emit("card_effect", self.id, member, "guardian_mitigation", 1, {
                            "mitigation_bonus": 1,
