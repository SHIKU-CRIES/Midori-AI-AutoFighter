        ...
        for filename in os.listdir(past_lives_folder):
            if filename.endswith(".pastlife"):
                ...
                try:
                    ...
                    self.MHP: int = self.MHP + self.check_base_stats(self.MHP, int(past_life_data['MHP']) + 100)
                    self.HP: int = self.MHP
                    ...
                    if past_life_data['Vitality'] < 0:
                        ...
                        if self.Vitality > 2:
                            self.Vitality = self.Vitality + ((past_life_data['Vitality'] - 1) / 250)
                        else:
                            self.Vitality = self.Vitality + max(((past_life_data['Vitality'] - 1) / 100), 0.001) + 0.0001
