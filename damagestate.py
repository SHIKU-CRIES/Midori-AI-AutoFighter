    ...
    if themed_names[0] in source.PlayerName.lower():
        ...
        if themed_names[0] in target.PlayerName.lower():
            ...
            if source.HP > source.MHP * 0.25:
                ...
                target.gain_damage_over_time(damageovertimetype("Bleeding Light", mited_damage_dealt ** 1.25, 55000, source.Type, source.PlayerName, 5), source.EffectHitRate)
            else:
                mited_damage_dealt = mited_damage_dealt * (((source.MHP - source.HP) + 1) * 2)
                target.gain_damage_over_time(damageovertimetype("Bleeding Light", mited_damage_dealt ** 1.35, 25000, source.Type, source.PlayerName, 2), source.EffectHitRate)
