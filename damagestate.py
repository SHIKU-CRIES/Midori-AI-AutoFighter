...
def apply_damage_item_effects(source: Player, target: Player, mited_damage_dealt: float):
        ...
        for item in source.Items:
            if not item:
                continue
            try:
                log(red, f"Item effect from {source.PlayerName}: {item.name}, Before Damage: {mited_damage_dealt}")
                mited_damage_dealt = item.on_damage_dealt(mited_damage_dealt)
                source.DamageDealt += int(mited_damage_dealt)
                log(red, f"Item effect from {source.PlayerName}: {item.name}, After Damage: {mited_damage_dealt}")
        ...
        for item in target.Items:
            if not item:
                continue
            try:
                log(green, f"Item effect from {target.PlayerName}: {item.name}, Before Damage: {mited_damage_dealt}")
                mited_damage_dealt = item.on_damage_taken(mited_damage_dealt)
                target.DamageTaken += int(mited_damage_dealt)
                log(green, f"Item effect from {target.PlayerName}: {item.name}, After Damage: {mited_damage_dealt}")
            except Exception as error:
                continue
...
def take_damage(source: Player, target: Player, fight_env_list: list):
    ...
    if (target.DodgeOdds / enrage_buff) >= random.random():
        ...
        target.HP -= int(mited_damage_dealt)