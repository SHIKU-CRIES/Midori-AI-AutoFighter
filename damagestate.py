...
def take_damage(source: Player, target: Player, fight_env_list: list, def_mod: float):
    ...
    if (target.DodgeOdds / enrage_buff) >= random.random():
        ...
        source_vit = ((source.Vitality ** 2) / def_mod)
        target_vit = ((target.Vitality ** 15) / def_mod)
        def_val = ((target.Def ** 2) / def_mod)
        ...
        if source.CritRate >= random.random():
            mited_damage_dealt = float(((damage_dealt * enrage_buff) / max(def_val * target_vit, 2)) * source.CritDamageMod) * max(1, source.CritRate)
            ...
            mited_damage_dealt = float((damage_dealt * enrage_buff) / max(def_val * target_vit, 2))
            mited_damage_dealt = mited_damage_dealt * random.uniform(0.95, 1.05)
            mited_damage_dealt = apply_damage_item_effects(source, target, mited_damage_dealt)
