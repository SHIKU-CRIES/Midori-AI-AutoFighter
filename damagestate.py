    ...
    if themed_names[0] in source.PlayerName.lower():
        ...
        if themed_names[0] in target.PlayerName.lower():
            ...
            if source.HP > source.MHP * 0.25:
                ...
                target.DOTS.append(damageovertimetype("Bleed", mited_damage_dealt * 25, 10, "generic", source.PlayerName, 5))
            else:
                mited_damage_dealt = mited_damage_dealt * (((source.MHP - source.HP) + 1) * 2)
                target.DOTS.append(damageovertimetype("Bleed", mited_damage_dealt * 10, 10, "generic", source.PlayerName, 2))
